...
- **Visualization data is signals, not events.** Ephemeral streams carry latest-value samples with user-defined shapes; there is no observation-event tier. Explicit UI/headless input events use a separate global action registry; they are not visualization streams. Consolidation that matters for an honest display — occurrence lists when several things happen between updates, min/max decimation of fast signals — is piece logic, published by the process as just another value. Intermediate values die server-side under conflating transport, so consolidation could not live anywhere else anyway.
...
- **Project libraries are normal code; packages are earned.** A project may carry its own library files beside its modules: helpers, renderers, algorithms, and codegen output that modules import by relative path. They are ordinary TypeScript, copy-pasteable between projects, and can be platform-agnostic, browser-specific, or Deno/native-specific. A library is not a module: it is not visualized, not instrumented, has no manifest entry, and is not held to the reload-without-page-reload expectation that visualized modules carry (picking up a library edit may need a page reload or engine restart). Tooling must not force a library to pose as a module to be type-checked or served. Only a library that proves generally useful across pieces is promoted to a top-level package, and that promotion is a deliberate act, not the default home for new code.
