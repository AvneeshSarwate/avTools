...
/**
 * Uncompress a snappy compressed buffer into an existing output buffer.
 *
 * @param {Uint8Array} input
 ...
 * @returns {Uint8Array}
 */
export function snappyUncompressInto(input, output, outputOffset = 0, outputLength = output.byteLength - outputOffset) {
  return snappyUncompressorInto()(input, output, outputOffset, outputLength)
}
...
export function snappyUncompressor() {
  ...
  return (input, outputLength) => {
    ...
    uncompressInto(input, output, 0, outputLength)
    return output
  }
}

/**
 * Load wasm and return uncompressor function that writes into caller-owned output.
 *
 * @returns {(input: Uint8Array, output: Uint8Array, outputOffset?: number, outputLength?: number) => Uint8Array}
 */
export function snappyUncompressorInto() {
  // Instantiate wasm module
  const wasm = instantiateWasm()

  return (input, output, outputOffset = 0, outputLength = output.byteLength - outputOffset) => {
    /** @type {any} */
    ...
    if (!Number.isInteger(outputOffset) || outputOffset < 0) {
      throw new RangeError('outputOffset must be a non-negative integer')
    }
    if (!Number.isInteger(outputLength) || outputLength < 0) {
      throw new RangeError('outputLength must be a non-negative integer')
    }
    if (outputOffset + outputLength > output.byteLength) {
      throw new RangeError('output range exceeds output buffer length')
    }
    ...
    // Copy uncompressed data from WASM memory into caller-owned output.
    output.set(byteArray.subarray(outputStart, outputStart + outputLength), outputOffset)
    return output
  }
}
