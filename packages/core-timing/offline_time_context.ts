...
const createYieldToMacrotask = (setTimeoutFn: SetTimeoutFn): (() => Promise<void>) => {
  ...
  return () => new Promise<void>((res) => setTimeoutFn(res, 0));
};
...
export class TimeScheduler {
  ...
  private readonly yieldToMacrotask: () => Promise<void>;
  private readonly scheduleMacrotask: (cb: () => void) => void;
  ...
  constructor(
    ...
    this.clearTimeoutFn = opts?.clearTimeout ?? DEFAULT_CLEAR_TIMEOUT;
    this.yieldToMacrotask = createYieldToMacrotask(this.setTimeoutFn);
    this.scheduleMacrotask = createScheduleMacrotask(this.setTimeoutFn);
    if (mode === "realtime") {
...
export function launch<T>(
  ...
  const scheduler = new TimeScheduler("realtime", {
    rate: opts?.rate ?? 1,
