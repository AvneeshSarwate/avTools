...
export interface BuiltParamSystem {
  ...
  setupPane(pane: PaneContainer): Map<string, PaneBinding>;
}
...
function cloneTrackElement(
  ...
  if (
    typeof rawValue === "object" && rawValue !== null && "funcName" in rawValue
  ) {
...
function buildTrackInput(name: string, def: ParamDef): TrackInput {
  if (isNumberParamDef(def)) {
    if (!Number.isFinite(def.min) || !Number.isFinite(def.max)) {
      throw new Error(
        `Number param "${name}" must define finite min/max bounds`,
      );
    }
...
function insertOrUpdateElementAtTime(
  ...
  const existingIndex = elementData.findIndex((element) =>
    Math.abs(element.time - time) < SNAPSHOT_TIME_EPS
  );

  if (track.fieldType === "number") {
    const nextElement: NumberElement = existingIndex >= 0
      ? {
        ...(elementData[existingIndex] as NumberElement),
        time,
    ...
    const nextElement: EnumElement = existingIndex >= 0
      ? {
        ...(elementData[existingIndex] as EnumElement),
        time,
        value: String(value),
      }
...
export function buildParamSystem(paramDefs: ParamDefs): BuiltParamSystem {
      ...
      if (isParamFolder(node)) {
        for (const [childKey, childValue] of Object.entries(node)) {
          ...
          if (childKey === "_actions") {
            for (
              const [actionKey, actionDef] of Object.entries(childValue ?? {})
            ) {
  ...
  const setupPane = (pane: PaneContainer): Map<string, PaneBinding> => {
      ...
      for (const [key, node] of Object.entries(defs)) {
        if (isParamFolder(node)) {
          ...
          for (const [childKey, childValue] of Object.entries(node)) {
            ...
            if (childKey === "_actions") {
              const actions = childValue as
                | Record<string, ActionDef>
                | undefined;
              for (
                const [actionKey, actionDef] of Object.entries(actions ?? {})
              ) {
                folder.addButton({ title: actionDef.label ?? actionKey }).on(
                  "click",
                  actionDef.action,
                );
              }
...
export function snapshotToAnimation(
  ...
  const updatedTracks = animation.tracks.map((track) => {
    ...
    if (isBooleanParamDef(paramDef)) {
      return insertOrUpdateElementAtTime(
        track,
        time,
