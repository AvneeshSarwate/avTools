...
import type { TrackRuntime, EditorAction, EnumElement } from '../types'
import {
  ENUM_LANE_HEIGHT,
  EDIT_LANE_BG_COLOR,
  EDIT_MARKER_WIDTH,
...
function drawTrackMarkers(track: TrackRuntime, isFront: boolean) {
  const elements = track.elementData as EnumElement[]
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    const color = hashStringToColor(elem.value)
    ...
    if (isFront) {
      ...
      group.on('click', (e) => {
        if (e.evt.shiftKey) {
          emit('action', { type: 'ENUM/DELETE', trackId: track.id, elementId: elem.id })
        } else {
          emit('action', { type: 'ELEMENT/SELECT', fieldType: 'enum', trackId: track.id, elementId: elem.id })
        }
      })
