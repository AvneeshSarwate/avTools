...
declare function defineCallback<T>(cb: (block: (ctx: TimeContext) => Promise<T>) => CancelablePromiseProxy<T>): void;

 declare class CancelablePromiseProxy<T> implements Promise<T> {
  promise?: Promise<T>;
 ...
 declare function launch<T>(block: (ctx: TimeContext) => Promise<T>): CancelablePromiseProxy<T>;
 declare abstract class TimeContext {
  ...
  branchWait<T>(block: (ctx: ToneTimeContext) => Promise<T>): CancelablePromiseProxy<T>;
...
export declare class CancelablePromiseProxy<T> implements Promise<T> {
  promise?: Promise<T>;
  abortController: AbortController;
  constructor(ab: AbortController);
  cancel(): void;
...
export declare function launch<T>(block: (ctx: TimeContext) => Promise<T>): CancelablePromiseProxy<T>;
...
export declare const chanExports: {
  CancelablePromiseProxy: typeof CancelablePromiseProxy;
