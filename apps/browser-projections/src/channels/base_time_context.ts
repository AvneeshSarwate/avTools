...
export class CancelablePromiseProxy<T> implements Promise<T> {
  ...
  [Symbol.toStringTag]: string = '[object CancelablePromiseProxy]'
...
export abstract class TimeContext {
  ...
  // public segmentWait<T>(block: (prog: number) => void, segmentBeats: number, frameTime: number = 0, debugName: string = ""): CancelablePromiseProxy<void> {
