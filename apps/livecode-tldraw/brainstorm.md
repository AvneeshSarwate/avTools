...
## internal "software engineering" architecture goal
- ECS flavored global data store
- have some kind of redux sagas (but cleaner and more minimal) action/dispatch based state editting to allow for clean headless operation (especially by agent, and this could even let you audit action plans generated by agent to steer system)
- tldraw is only views onto that store - creating/destroying tldraw shapes only creates/destorys those views, not underlying entities
  - this maybe lets you separate tldraw undo/redo from data-store undo/redo

## how to build a robust control signal visualization + orchestrating environment?
- eg, something like chops, but that also works well with the timeline-editor component, and the type of arbitrary-loop driven timing you can do with the TimeContext engine - need both read-stream type stuff for render loops that just grab the value at frame time, vs react-to-change type stuff for trigger based musical event scheduling
- getting this right mean creating a "more than the sum of its parts" interaction between the GUI composed things (piano rolls, animation editor, etc) vs procedural things wiht TimeContext
- maybe even have some kind of state machine builder/runner/visualizer