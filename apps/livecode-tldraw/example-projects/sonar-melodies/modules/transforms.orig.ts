...
export function segmentByPitchMarker(
  ...
  markers.forEach((marker, idx) => {
    ...
    const fullDur = fullNotes.reduce(
      (m, n) => Math.max(m, n.position + n.duration),
      0,
    );
    ...
    const clippedDur = clippedNotes.reduce(
      (m, n) => Math.max(m, n.position + n.duration),
      0,
    );
...
export function sliceAndTransposeByMarkers(
  ...
  defs.forEach((def, sliceIdx) => {
    ...
    slice.duration = def.quantization == 0
      ? slice.duration
      : Math.ceil(slice.duration / def.quantization) * def.quantization;
...
export function retrogradeClip(
  clip: AbletonClip,
  enable: number = 1,
): AbletonClip {
  ...
  const maxEnd = result.notes.reduce(
    (max, n) => Math.max(max, n.position + n.duration),
    0,
  );
  if (maxEnd > result.duration) {
    console.warn(
      `[retrogradeClip] Notes extend beyond duration: ${maxEnd} > ${result.duration}`,
    );
  }
...
export function invertClip(
  ...
  if (maxEnd > result.duration) {
    console.warn(
      `[invertClip] Notes extend beyond duration: ${maxEnd} > ${result.duration}`,
    );
  }
...
export function accentClip(
  ...
  accentedNote.velocity = Math.min(
    127,
    Math.round(accentedNote.velocity * velocityMultiplier),
  );
...
export function timeStretch(clip: AbletonClip, factor: number): AbletonClip {
  ...
  if (maxEnd > result.duration) {
    console.warn(
      `[timeStretch] Notes extend beyond duration: ${maxEnd} > ${result.duration}`,
    );
  }
...
export function endTimeQuantize(
  clip: AbletonClip,
  quantValue: number,
): AbletonClip {
...
export function scaleTranspose(
  ...
  if (maxEnd > result.duration) {
    console.warn(
      `[scaleTranspose] Notes extend beyond duration: ${maxEnd} > ${result.duration}`,
    );
  }
...
export function scaleTransposeOneNote(
  clip: AbletonClip,
  transpose: number,
  noteInd: number,
  scale?: Scale,
  ...
  newClip.notes[noteInd].pitch = scale.getByIndex(
    Math.round(sourceIndex + transpose),
  );
  return newClip;
}
...
export function arpeggiateClip(
  ...
  const pattern = typeof patternArg === "string"
    ? patternArg.toLowerCase()
    : arpeggioPatterns[patternArg % arpeggioPatterns.length];
  ...
  noteGroups.forEach((notes, position) => {
    ...
    const patternIndices = generateArpeggioIndices(
    ...
    for (let step = 0; step < steps; step++) {
      ...
      while (currentTime < chordDuration) {
        ...
        const originalNote = notes.find((n) =>
          n.pitch === sortedPitches[pitchIndex]
        ) || notes[0];
  ...
  const lastNoteEnd = arpeggiatedNotes.reduce(
...
export function velocityMultiply(
  clip: AbletonClip,
  factor: number,
): AbletonClip {
  const newClip = clip.clone();
...
export function harmonizeClip(
  ...
  const degrees = [degree1, degree2, degree3, degree4].filter((
    d,
  ): d is number => d !== undefined && d !== 0);
...
export function segment(clip: AbletonClip, index: number): AbletonClip {
  ...
  const wrappedIndex = ((index % segments.length) + segments.length) %
    segments.length;
...
export function durSlice(clip: AbletonClip, start: number, duration: number) {
  ...
  if (duration >= clip.duration) {
    ...
    console.log(
      "duration is greater than clip duration, looping",
      numLoops,
      "times",
    );
  } else {
...
export function nnotes(
  clip: AbletonClip,
  n: number,
  start: number = 0,
): AbletonClip {
  const nSortedNotes = clip.notes.map((n) => ({ ...n })).sort((a, b) =>
    a.position - b.position
  ).slice(start, start + n);
  ...
  for (let i = 0; i < nSortedNotes.length; i++) {
    newDuration = Math.max(
      newDuration,
      nSortedNotes[i].position + nSortedNotes[i].duration,
    );
  }
...
export function easeCirc(clip: AbletonClip, amount: number): AbletonClip {
  ...
  // Check if any notes extend beyond clip duration
  const maxEnd = newClip.notes.reduce(
    (max, n) => Math.max(max, n.position + n.duration),
    0,
  );
  if (maxEnd > newClip.duration) {
    console.warn(
      `[easeCirc] Notes extend beyond duration: ${maxEnd} > ${newClip.duration}`,
    );
  }
...
export function repeatNotes(clip: AbletonClip, n: number): AbletonClip {
  ...
  for (let i = 0; i < sortedNotes.length; i++) {
    ...
    const gapToNext = nextNote
      ? (nextNote.position - (note.position + note.duration))
      : 0;
...
export function rotateClip(
  ...
  if (maxEnd > result.duration) {
    console.warn(
      `[rotateClip] Notes extend beyond duration: ${maxEnd} > ${result.duration}`,
    );
  }
...
export function ornamentClip(
  ...
  sortedNotes.forEach((note) => {
    ...
    if (note.duration <= 0.25 || random() > probability) {
      newNotes.push({
        ...note,
    ...
    const ornamentType = Math.floor(random() * 4);
    const durFactor = 0.25 + random() * 0.5;
  ...
  const newDuration = newNotes.reduce(
    (max, n) => Math.max(max, n.position + n.duration),
    0,
  );
...
export const scaleMap: Record<string, Scale> = {
  C: new Scale(),
...
export function silenceClip(clip: AbletonClip, duration: number) {
