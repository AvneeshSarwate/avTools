...
library's own test suite (`packages/core-timing/timing_tests.ts`). Read them
in order: one timeline and logical time, then many timelines and how they
join, then how timelines that never join stay coordinated, then how a subtree
of timelines is torn down, and finally how time itself becomes musical.
...
3. **barrier**: A's bar fills, then shows hatching while B finishes; A's next
...
4. **cancel**: two dots orbit for `lifetimeSec`, vanish on cancel, "ticks
