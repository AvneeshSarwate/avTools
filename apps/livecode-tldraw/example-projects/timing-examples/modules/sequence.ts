...
export async function runFunc (ctx: TimeContext) {
  ...
  const runScene = async (c: TimeContext) => {
    ...
    try {
      for (let i = 0;; i++) {
        ...
        await __tcvVisualizedAwait("timing/sequence", "dd7c41c5-f1ee-43ea-b1de-d6e3058cb4bf", c.waitSec(params.stepSec));
      }
    } finally {
  ...
  while (true) {
    await __tcvVisualizedAwait("timing/sequence", "d7af90fd-291f-467a-91d1-49dde66e5dde", ctx.waitSec(1 / 60));
