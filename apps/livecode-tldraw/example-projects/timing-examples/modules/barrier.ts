...
export async function runFunc (ctx: TimeContext) {
  ...
  const phrase = async (v: TimeContext, voice: Voice, seconds: number) => {
    ...
    while (v.time - start < seconds) {
      ...
      await __tcvVisualizedAwait("timing/barrier", "66893968-81a0-4380-8f20-30051f7ea22c", v.waitSec(1 / 60));
    }
    voice.progress = 1;
    voice.cycles += 1;
  };
  ...
  const runScene = async (c: TimeContext) => {
    c.branch(async (b) => {
      while (true) {
        startBarrier(KEY, b);
        await __tcvVisualizedAwait("timing/barrier", "7e919343-8b4d-4a49-948c-906ecc52c35e", phrase(b, state.b, params.bPhraseSec));
    ...
    c.branch(async (a) => {
      while (true) {
        await __tcvVisualizedAwait("timing/barrier", "cfd7e4c4-5584-4234-8f94-bb1b6370c5c7", phrase(a, state.a, params.aPhraseSec));
        state.a.waiting = true;
        await __tcvVisualizedAwait("timing/barrier", "a4330911-0f9d-4a17-b574-f4de2ffb83fd", awaitBarrier(KEY, a));
    ...
    while (true) await __tcvVisualizedAwait("timing/barrier", "ab419b2a-8b69-4ec9-a872-9869279e654d", c.waitSec(3600));
  };

  while (true) {
    await __tcvVisualizedAwait("timing/barrier", "93ec97e0-56c4-484b-bbf1-0ee2873a910c", ctx.waitSec(1 / 60));
