...
| `timing-examples` | Gallery of the timing library's control topologies for a baked single-page demo: drift-free sequential waits, branch fan-out and join, cancellation cascades, barrier sync between loops of different lengths, and beat waits under live tempo changes (shared vs cloned tempo, rubato ramps). Five independent modules, each with a canvas view and a `running` toggle. |
