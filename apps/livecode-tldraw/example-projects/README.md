...
| `feature-midi-recording` | MIDI input slice: a params input selector refreshed from live ports, a recording toggle, and start/end silence trimming, writing each take, with per-note pitch bend, pressure and timbre, to one piano roll. Runs on either engine. |
| `feature-midi-recording-six-sines` | Browser-engine companion of `feature-midi-recording`: the same recorder, plus a player that performs the take on Six Sines with per-note pitch, pressure and timbre as CLAP note expressions, starting on an MPE factory preset. |
...
| `timing-examples` | Gallery of the timing library's control topologies for a baked single-page demo: drift-free sequential waits, branch fan-out and join, barrier sync between loops of different lengths, cancellation cascades, and beat waits under live tempo changes (shared vs cloned tempo, rubato ramps). Five independent modules, each with a canvas view and a `running` toggle. |
...
| `feature-drawing-radiance-cascades` | 2D radiance cascades on WebGPU over the drawing's outlines **with checked-in `data/`** (browser-engine target, `engine=inprocess`): a project library (`lib/`) of hand-written shader-fx effects, per-shape `metadata` for stroke width, emission, transmittance, and albedo, a params pane for every cascade parameter, merge fixes, bounce, sky, debug views, and a brute-force reference; WGSL validated by `naga`, rendered headlessly by a Deno check. |
