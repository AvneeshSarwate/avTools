...
/**
 * One durable entity's saved JSON file. Deliberately top-level rather than
 * inside `canvas`, which `/project/canvas` whole-replaces. The entry carries
 * the true entity name, so the encoded filename never has to be decoded.
 */
...
/**
 * Informational, warning-tier dirty state for one durable entity. A
 * saved-but-absent entity reports as an unsaved deletion. Nothing ever
 * auto-saves off the back of this.
 */
