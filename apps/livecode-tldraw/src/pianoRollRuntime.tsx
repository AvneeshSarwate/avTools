...
export function usePianoRollRuntime() {
  ...
  if (!runtime) {
    throw new Error('usePianoRollRuntime must be used inside PianoRollRuntimeProvider')
  }
  return runtime
}
...
export function PianoRollRuntimeProvider({
  ...
  const [connectionStatus, setConnectionStatus] = useState<PianoRollConnectionStatus>('closed')
  ...
  useEffect(() => {
    ...
    const socketUrl = `${serverBaseUrl.replace(/^http/, 'ws')}/piano-roll/snapshots`
    ...
    socket.onmessage = (event) => {
      pendingSnapshotRef.current = JSON.parse(event.data as string) as PianoRollSnapshot
