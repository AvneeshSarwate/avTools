...
export class LivecodeEditorShapeUtil extends BaseBoxShapeUtil<LivecodeEditorShape> {
...
function LivecodeEditorShapeComponent({ shape }: LivecodeEditorShapeComponentProps) {
  const editor = useEditor()
  const runtime = useLivecodeRuntime()
  const { setModuleSource } = runtime
  const moduleState = runtime.modules[shape.props.moduleId]
  const documentUri = useMemo(() => livecodeDocumentUri(shape.props.moduleId), [shape.props.moduleId])
    ...
    >
      <div className="livecode-shape__header">
        <div className="livecode-shape__title">
          ...
          <span>{shape.props.moduleId}</span>
        </div>
        <div className="livecode-shape__actions" onPointerDown={(event) => event.stopPropagation()}>
          <button
            type="button"
            disabled={runtime.connectionStatus !== 'open' || runStatus === 'running'}
          ...
          <button
            type="button"
            disabled={runtime.connectionStatus !== 'open' || runStatus === 'stopping'}
      ...
      <div className="livecode-shape__meta">
        <span className={`status-pill status-pill--${buildStatus}`}>build: {buildStatus}</span>
        <span className={`status-pill status-pill--${runStatus}`}>run: {runStatus}</span>
      ...
      <div className="livecode-shape__footer" onPointerDown={(event) => event.stopPropagation()}>
        {diagnostics.length > 0 ? (
          diagnostics.slice(0, 2).map((diagnostic) => (
            <div key={`${diagnostic.code}-${diagnostic.from}-${diagnostic.to}`} className="diagnostic">
              {diagnostic.message}
            </div>
          ))
        ) : lspDiagnostics.length > 0 ? (
          lspDiagnostics.slice(0, 2).map((diagnostic, index) => (
            <div key={`${diagnostic.code ?? 'lsp'}-${index}`} className="diagnostic">
              {diagnostic.message}
            </div>
          ))
          ...
          <div className="diagnostic diagnostic--quiet">
            snapshot {moduleState?.lastSnapshotSeq ?? '-'} / source v{moduleState?.sourceVersion ?? 0}
          </div>
        )}
      </div>
    </HTMLContainer>
