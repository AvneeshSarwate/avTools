...
# -----------------------------------------------------------------------------
# 6. Optional: install avtools Deno kernel spec (if jupyter is available)
# -----------------------------------------------------------------------------
if [ -f "apps/deno-notebooks/scripts/install_avtools_kernel.sh" ]; then
  if command -v jupyter &> /dev/null; then
    echo "[6/5] Installing avtools Deno kernel spec..."
    bash apps/deno-notebooks/scripts/install_avtools_kernel.sh
    echo ""
  else
    echo "Skipping kernel install (jupyter not found)."
    echo ""
  fi
fi
